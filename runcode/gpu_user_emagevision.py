file = open('../emagevision/output_show_user_gpu_usage.log', 'r')
lines = file.read().splitlines()
file.close()

# write to excel
import xlsxwriter
workbook = xlsxwriter.Workbook('../emagevision/emagevision_gpu_usage.xlsx')
...
while (_count < _len_lines):
    ...
    if date_line.split(' ')[1] in _arr_mouth:
        ...
        _count += 2
        print (lines[_count])
        print (lines[_count + 4])
        _len_of_gpu = 8
        _sub_count = 0
        while (_sub_count < _len_of_gpu):
            print ('---')
            print (lines[_count + _sub_count].split('|'))
            get_name = lines[_count + _sub_count].split('|')[3].strip()
            get_percentage = lines[_count + _sub_count].split('|')[1].strip().split(',')[1].strip().split(' ')[0] + "%"
            print (get_percentage)
            print (get_name)
            get_name = get_name.split('(')[0] + get_percentage
